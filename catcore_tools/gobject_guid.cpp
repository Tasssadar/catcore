...
=============================================================
 ...
 For compile use:
     g++ -o gobject_guid $(mysql_config --cflags) gobject_guid.cpp $(mysql_config --libs) -g
...
int main()
{
    ...
    file << "gobject_guidlog_" << time(NULL);
    ...
    if (mysql_query(conn, "SELECT guid FROM gameobject ORDER BY guid")) {
        fprintf(log, "%s\n", mysql_error(conn));
        return 1;
    }
    ...
    fprintf(log, "============================ \n");
    //Cleanup...
    mysql_query(conn, "SELECT guid FROM gameobject WHERE guid=0");
    ...
    if(row != NULL)
    {
        printf("\n coze? \n");
        std::stringstream ss;
        ss << "UPDATE gameobject SET guid = " << *(free.begin()) <<  " WHERE guid=0"; 
        fprintf(log, "\n %s \n", ss.str().c_str());
        mysql_free_result(res);
    ...
    for(std::list<long>::iterator itr = guids.begin(); itr != guids.end(); ++itr)
    {
        ...
        for(int y = 0; y < 3; ++y)
        {
             ...
             if(row = mysql_fetch_row(res))
             {
                 ...
                 {
                    fprintf(log, "\n edit %s for gameobject %u", (tabulky[y].name).c_str(), *itr);
                    mysql_free_result(res);
        ...
        if(!can)
            continue;
        std::stringstream ss;
        ss << "UPDATE gameobject SET guid = " << *(free.begin()) << " WHERE guid =  " << *itr;
        fprintf(log, "\n \n %s", ss.str().c_str());
    ...
    mysql_query(conn, "SELECT MAX(guid) FROM gameobject");
    ...
    printf("\n Changed gameobjects: %u, maxguid: %u, space between overflow: %u\n", counta, maxguid, (0x00FFFFFF - maxguid));
    mysql_free_result(res);
    mysql_close(conn);
    fclose(log);
}
