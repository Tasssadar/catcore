...
struct MANGOS_DLL_DECL boss_brutallusAI : public ScriptedAI
{
    ...
    void Aggro(Unit* pWho)
    {
        ...
        Creature* Madrigosa = m_creature->SummonCreature(CREATURE_MADRIGOSA, 1465.8f, 647.0f, m_creature->GetPositionZ(), 4.7f, TEMPSUMMON_TIMED_DESPAWN, 42000);
    }
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (m_uiBurnTimer < uiDiff)
        {
            ...
            {
                DoCast(pTarget,SPELL_BURN);
                pTarget->CastSpell(pTarget,SPELL_BURN_AURA, true);
            }
            m_uiBurnTimer = 60000;
        }
