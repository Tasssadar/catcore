...
#define GOSSIP_ITEM_MUX_1 "Extra-Dimensional Ghost Revealer"
...
bool GossipHello_npc_mux_manascrambler(Player* pPlayer, Creature* pCreature)
{
    ...
    if (pPlayer->GetQuestStatus(8977) == QUEST_STATUS_INCOMPLETE || pPlayer->GetQuestStatus(8978) == QUEST_STATUS_INCOMPLETE
        || pPlayer->GetQuestStatus(8977) == QUEST_STATUS_COMPLETE || pPlayer->GetQuestStatus(8978) == QUEST_STATUS_COMPLETE)
    {
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_MUX_1, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
    }
    pPlayer->SEND_GOSSIP_MENU(8335, pCreature->GetGUID());
...
bool GossipSelect_npc_mux_manascrambler(Player* pPlayer, Creature* pCreature, uint32 uiSender, uint32 uiAction)
{
    switch(uiAction)
    {
        case GOSSIP_ACTION_INFO_DEF:
             pPlayer->CLOSE_GOSSIP_MENU();
             pCreature->CastSpell(pPlayer,27754,false);
        break;
...
void AddSC_tanaris()
{
    ...
    newscript = new Script;
    newscript->Name = "npc_mux_manascrambler";
    newscript->pGossipHello = &GossipHello_npc_mux_manascrambler;
    newscript->pGossipSelect = &GossipSelect_npc_mux_manascrambler;
    newscript->RegisterSelf();
}
