...
// Brewfest keging
// give keg to player
struct MANGOS_DLL_DECL mob_keg_addAI : public ScriptedAI
{
    mob_keg_addAI(Creature* pCreature) : ScriptedAI(pCreature) { Reset(); }
    ...
    void MoveInLineOfSight(Unit* pWho)
    {
        ...
        if (m_creature->IsWithinDist(pWho, 10))
        {
            if (!pWho->HasAura(43880) || pWho->GetTypeId() != TYPEID_PLAYER)
                return;

            Player* player = (Player*)pWho;
            if (player->HasItemCount(33797, 1))
                return;

            uint32 entry = player->GetTeam() == ALLIANCE ? 11122 : 11412;
            uint32 quest_id1 = player->GetTeam() == ALLIANCE ? 11293 : 11408;
            uint32 quest_id2 = player->GetTeam() == ALLIANCE ? 11294 : 11407;
            if (player->GetQuestStatus(entry) != QUEST_STATUS_NONE &&
                player->GetQuestStatus(quest_id1) != QUEST_STATUS_INCOMPLETE &&
                player->GetQuestStatus(quest_id2) != QUEST_STATUS_INCOMPLETE)
                m_creature->CastSpell(pWho, 43660, true);
        }
    }

};
CreatureAI* GetAI_mob_keg_add(Creature* pCreature)
{
    return new mob_keg_addAI(pCreature);
}
// take keg from player
struct MANGOS_DLL_DECL mob_keg_takeAI : public ScriptedAI
{
    mob_keg_takeAI(Creature* pCreature) : ScriptedAI(pCreature) { Reset(); }
    ...
    void MoveInLineOfSight(Unit* pWho)
    {
        ...
        if (m_creature->IsWithinDist(pWho, 15))
        {
            if (!pWho->HasAura(43880) || pWho->GetTypeId() != TYPEID_PLAYER)
                return;
            ...
            pWho->CastSpell(m_creature, 43662, true);
            if (player->HasItemCount(33797, 1))
            {
                error_log("Brewfest:: Item was not destroyed !");
                player->DestroyItemCount(33797, 1, true);
            }
            uint32 entry = player->GetTeam() == ALLIANCE ? 11122 : 11412;
            if (player->GetQuestRewardStatus(entry))
            {
                if (Aura* pAura = player->GetAura(43880, EFFECT_INDEX_0))
                {
                    pAura->SetAuraDuration(pAura->GetAuraDuration() + 15000);
                    pAura->SendAuraUpdate(false);
                }
                player->CastSpell(player, 44501, true);
            }
        }
    }

};
CreatureAI* GetAI_mob_keg_take(Creature* pCreature)
{
    return new mob_keg_takeAI(pCreature);
}
...
void AddSC_orgrimmar()
{
    ...
    newscript = new Script;
    newscript->Name = "mob_keg_add";
    newscript->GetAI = &GetAI_mob_keg_add;
    newscript->RegisterSelf();

    newscript = new Script;
    newscript->Name = "mob_keg_take";
    newscript->GetAI = &GetAI_mob_keg_take;
    newscript->RegisterSelf();
}