...
float fCoord[4][4] =
{
    {296.49f, -346.46f, 90.54f, 0.0f},
    {299.56f, -343.73f, 90.55f, 3.93f},
    {293.81f, -343.33f, 90.52f, 5.34f},
    {296.49f, -349.22f, 90.55f, 1.57f},
};
...
struct MANGOS_DLL_DECL boss_svalaAI : public ScriptedAI
{
    ...
    {
        if (m_creature->HasSplineFlag(SPLINEFLAG_NO_SPLINE))
            ...
            if (Unit* pPlayer = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0))
            {
                ...
                for(uint8 i=0; i<3; ++i)
                    if (Creature* pAdd = m_creature->SummonCreature(NPC_CHANNELER, fCoord[i][0], fCoord[i][1], fCoord[i][2], fCoord[i][3], TEMPSUMMON_TIMED_DESPAWN, 9000))
                    {
                        ...
                        //SPELL_PARALYZE has 20y radius :o
                        //pAdd->CastSpell(pPlayer, SPELL_PARALYZE, false);
                        const SpellEntry* stunProto = sSpellStore.LookupEntry(70583);
                        int32 basepoints = stunProto->EffectBasePoints[SpellEffectIndex(i)];
                        Aura *stun = CreateAura(stunProto, EFFECT_INDEX_0, &basepoints, pPlayer, pAdd);
                        pPlayer->AddAura(stun);
                    }
            }
            m_bIsSacrifice = true;
            m_uiSacrificeTimer = 20000;
