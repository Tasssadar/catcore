...
struct boss_palehoofAI : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        if (pInstance)
        {
            ...
            Creature* pTemp = m_creature->GetMap()->GetCreature(pInstance->GetData64(NPC_WORGEN));
            if (pTemp && !pTemp->isAlive())
                pTemp->Respawn();

            pTemp = m_creature->GetMap()->GetCreature(pInstance->GetData64(NPC_RHINO));
            if (pTemp && !pTemp->isAlive())
                pTemp->Respawn();

            pTemp = m_creature->GetMap()->GetCreature(pInstance->GetData64(NPC_JORMUNGAR));
            if (pTemp && !pTemp->isAlive())
                pTemp->Respawn();

            pTemp = m_creature->GetMap()->GetCreature(pInstance->GetData64(NPC_FURBOLG));
            if (pTemp && !pTemp->isAlive())
                pTemp->Respawn();

            GameObject* pGo = GetClosestGameObjectWithEntry(m_creature,GO_STASIS_GENERATOR,100.0f);
            if (pGo)
            {
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        Creature* pTemp = GetClosestCreatureWithEntry(m_creature,26688,100.0f);
        if (pTemp && pTemp->isAlive())
    ...
    void NextPhase()
    {
        if (currentPhase == PHASE_NONE)
        {
            ...
            m_creature->SummonCreature(MOB_STASIS_CONTROLLER,moveLocs[5].x,moveLocs[5].y,moveLocs[5].z,0,TEMPSUMMON_CORPSE_DESPAWN,0);
        }
        Phase move = PHASE_NONE;
        if (AddCount >= (m_bIsRegularMode ? 2 : 4))
        ...
        Creature* pOrb = GetClosestCreatureWithEntry(m_creature,26688,100.0f);
...
struct mob_ravenous_furbolgAI : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        if (pInstance)
            if (pInstance->GetData(TYPE_GORTOK) == IN_PROGRESS)
            {
                Creature *pPalehoof = GetClosestCreatureWithEntry(m_creature,NPC_GORTOK,100.0f);
                if (pPalehoof && pPalehoof->isAlive())
                    ((boss_palehoofAI*)pPalehoof->AI())->Reset();
            }
    }
    ...
    void JustDied(Unit* /*killer*/)
    {
        if (pInstance)
        {
            Creature *pPalehoof = GetClosestCreatureWithEntry(m_creature,NPC_GORTOK,100.0f);
            if (pPalehoof && pPalehoof->isAlive())
                ((boss_palehoofAI*)pPalehoof->AI())->NextPhase();
        }
    }
...
struct mob_palehoof_orbAI : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        m_creature->SetSpeedRate(MOVE_FLIGHT , 0.5f);
    }
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (SummonTimer <= diff)
        {
            if (currentPhase<5&&currentPhase >= 0)
            {
               ...
               switch(currentPhase)
               {
                    case PHASE_FRENZIED_WORGEN: pNext = m_creature->GetMap()->GetCreature(pInstance ? pInstance->GetData64(NPC_WORGEN) : 0); break;
                    case PHASE_RAVENOUS_FURLBORG: pNext = m_creature->GetMap()->GetCreature(pInstance ? pInstance->GetData64(NPC_FURBOLG) : 0); break;
                    case PHASE_MASSIVE_JORMUNGAR: pNext = m_creature->GetMap()->GetCreature(pInstance ? pInstance->GetData64(NPC_JORMUNGAR) : 0); break;
                    case PHASE_FEROCIOUS_RHINO: pNext = m_creature->GetMap()->GetCreature(pInstance ? pInstance->GetData64(NPC_RHINO) : 0); break;
                    case PHASE_GORTOK_PALEHOOF: pNext = GetClosestCreatureWithEntry(m_creature,NPC_GORTOK,100.0f);break;
               }
               ...
               if (pNext)
               {
                    ...
                    pNext->AI()->AttackStart(m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM,0));
...
bool GOHello_palehoof_sphere(Player * /*pPlayer*/, GameObject *pGO)
{
    ...
    Creature *pPalehoof = GetClosestCreatureWithEntry(pGO,NPC_GORTOK,100.0f);
    if (pPalehoof && pPalehoof->isAlive())
    {
