...
static LocationsXY PrePhaseAdds[]=
{
    {2149.68f, -263.48f, 419.68f},
    {2126.19f, -264.04f, 419.81f},
    {2120.25f, -267.47f, 419.67f},
    {2119.91f, -261.30f, 419.66f},
};
...
struct MANGOS_DLL_DECL boss_thorimAI : public ScriptedAI
{
    ...
    uint32 m_uiPrePhaseCheck;
    //bool m_uiPreAddsSpawned;
    ...
    {
        uint8 m_uiRandom = urand (0,1);
        uint32 captain =  m_uiRandom ? NPC_CAPTAIN_HORDE : NPC_CAPTAIN_ALY;
        uint32 mercenary = m_uiRandom ? NPC_MERCENARY_HORDE : NPC_MERCENARY_ALY;
        uint32 AddsOrder[4] = {NPC_JORMUNGAR_BEHEMOTH, captain, mercenary, mercenary};
        
        // Jormungar
        Creature* pJormungar = m_creature->SummonCreature(AddsOrder[0], PrePhaseAdds[0].x, PrePhaseAdds[0].y, PrePhaseAdds[0].z, M_PI_F, TEMPSUMMON_DEAD_DESPAWN, 0);
        if (pJormungar)
            m_lPrePhaseGUIDs[0] = pJormungar->GetGUID();

        // soldiers
        for (uint8 i = 1; i < 4; ++i)
        {
            float x = PrePhaseAdds[i].x;
            float y = PrePhaseAdds[i].y;
            float o = 0;

            if (pJormungar)
                o = pJormungar->GetAngle(x,y) + M_PI_F;

            if (o > 2*M_PI_F)
                o -= 2*M_PI_F;
            
            if (Creature* pTemp = m_creature->SummonCreature(AddsOrder[i], x, y, PrePhaseAdds[i].z, o, TEMPSUMMON_DEAD_DESPAWN, 0))
            {
                m_lPrePhaseGUIDs[i] = pTemp->GetGUID();
                if (pJormungar)
                    pTemp->AI()->AttackStart(pJormungar);
            }		
        }
    }
    ...
    void DoOutro()
    {
        if(m_pInstance) 
        {
            ...
            // spawn loot chest
            m_creature->SummonGameobject(m_bIsRegularMode ? LOOT_THORIM : LOOT_THORIM_H,
                m_fLootThorim[0], m_fLootThorim[1], m_fLootThorim[2], m_fLootThorim[3], 604800);
            if(m_bIsHardMode)
            {
                ...
                // spawn loot chest
                m_creature->SummonGameobject(m_bIsRegularMode ? LOOT_THORIM_HARD : LOOT_THORIM_HARD_H,
                    m_fLootThorimH[0], m_fLootThorimH[1], m_fLootThorimH[2], m_fLootThorimH[3], 604800);
            }
        }

        m_creature->SetHardModeKill(m_bIsHardMode);
        m_creature->LogKill(m_pKiller);
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        switch(m_uiPhase)
        {
            ...
            case PHASE_PREADDS:
            {
                if (m_uiPrePhaseCheck < uiDiff)
                {
                    bool m_bIsEveryoneDead = true;
                    for (uint8 i = 0; i < 4; ++i)
                    {
                        Creature* pUnit = (Creature*)Unit::GetUnit(*m_creature, m_lPrePhaseGUIDs[i]);
                        if (!pUnit || !pUnit->isAlive())
                            continue;

                        m_bIsEveryoneDead = false;
                        if (pUnit->GetHealthPercent() <= 80)
                            if (pUnit->getThreatManager().getPlayerThreatList().empty())
                                pUnit->SetHealth(pUnit->GetMaxHealth());
                    }
                    if (m_bIsEveryoneDead)
                        StartEncounter();
                    m_uiPrePhaseCheck = 2000;
                }else m_uiPrePhaseCheck -= uiDiff;
             break;
            }
            // do intro
            case PHASE_INTRO:
                ...
                if(m_uiPhase2Timer < uiDiff && m_bIsPhaseEnd)
                {
                    ...
                    if(m_bIsHardMode)
                    {
                        if(Creature* Sif = m_pInstance->instance->GetCreature(m_uiSifGUID))
                        {
                            ...
                            Sif->Relocate(PrePhaseAdds[0].x, PrePhaseAdds[0].y, PrePhaseAdds[0].z);
                            Sif->GetMotionMaster()->MoveChase(m_creature->getVictim());
                        }
                    }
...
struct MANGOS_DLL_DECL boss_runic_colossusAI : public ScriptedAI
{
    ...
    uint16 m_uiSmashCasted;
    uint8 m_uiSmashPhase;
    bool m_bAttacked;
    bool m_bCastLeft;
    bool m_bTargetsLoaded;
    std::list<DistCreature> LeftRunicSmash;
    std::list<DistCreature> RightRunicSmash;
    std::list<DistCreature>::iterator NowExploding;
    ...
    void DamageTaken(Unit* pAttacker,uint32 &damage)
    {
        ...
        {
            m_bAttacked = true;
            m_creature->AI()->AttackStart(pAttacker);
        }
    }
    
    void UpdateAI(const uint32 uiDiff)
    {
        if(m_uiSmashTimer < uiDiff && !m_bAttacked)
        {
            if (!m_bTargetsLoaded)
            {
            ...
            if (m_bCastLeft)
                DoCast(m_creature,SPELL_RUNIC_SMASH);
            else
                DoCast(m_creature,SPELL_RUNIC_SMASH2);
            m_uiSmashTimer = 10000;
            m_uiSmashCasted = 5500;
            m_uiSmashPhase = 0;
            NowExploding = (m_bCastLeft ? LeftRunicSmash.begin() : RightRunicSmash.begin());
        }
