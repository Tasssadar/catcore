...
struct MANGOS_DLL_DECL mob_voidzoneAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
        if (m_uiDespawn_Timer < diff)
        {
            ...
            m_uiDespawn_Timer = 5000;
        }else m_uiDespawn_Timer -= diff;

        if (m_uiDamage_Timer < diff)
        {
            uint32 entry = m_bIsRegularMode ? SPELL_VOIDZONE : SPELL_VOIDZONE_H;
            SpellEntry const* spellInfo = sSpellStore.LookupEntry(entry);
            uint32 damage = spellInfo->CalculateSimpleValue(EFFECT_INDEX_1); 
            ...
            if (map && map->IsDungeon())
            {
                ...
                if (!PlayerList.isEmpty())
                    for (Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
                    {
                        ...
                        if (plr && plr->isAlive() && m_creature->GetDistance2d(plr) < 6)
                        {
                            m_creature->SendSpellNonMeleeDamageLog(plr, entry, damage, SPELL_SCHOOL_MASK_SHADOW, 0, 0, false, 0, false);
                            m_creature->DealDamage(plr, damage, NULL, DIRECT_DAMAGE, SPELL_SCHOOL_MASK_SHADOW, NULL, false);
                        }
                    }
            }
            m_uiDamage_Timer = 1000;
        }else m_uiDamage_Timer -= diff;
    }
};
...
struct MANGOS_DLL_DECL mob_xtheartAI : public ScriptedAI
{
    ...
    void DamageTaken(Unit* pDoneBy, uint32& uiDamage)
    {
        ...
        // double damage --- should work due to aura
        //uiDamage += uiDamage;
    }
...
struct MANGOS_DLL_DECL boss_xt002AI : public ScriptedAI
{
    ...
    void JustDied(Unit* pKiller)
    {
        if (m_pInstance)
        {
            ...
            if(m_bIsHardMode)
            {
                ...
                m_creature->SummonGameobject( m_bIsRegularMode ? LOOT_XT002_HARD : LOOT_XT002_HARD_H, 
                   m_fLootXT002[0], m_fLootXT002[1], m_fLootXT002[2], m_fLootXT002[3], 604800);
            }
        ...
        std::list<Creature*> lVoidZone;
        GetCreatureListWithEntryInGrid(lVoidZone, m_creature, NPC_VOIDZONE, DEFAULT_VISIBILITY_INSTANCE);
        if (!lVoidZone.empty())
        {
            for(std::list<Creature*>::iterator iter = lVoidZone.begin(); iter != lVoidZone.end(); ++iter)
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if(m_bIsHardMode)
        {
            ...
            {
                Unit* pTarget = m_creature->SelectAttackingPlayer(ATTACKING_TARGET_RANDOM, 1);
                if (!pTarget)
                    pTarget = m_creature->getVictim();
                
                Creature* pLifeSpark = m_creature->SummonCreature(NPC_LIFESPARK, pTarget->GetPositionX(), pTarget->GetPositionY(), pTarget->GetPositionZ(), 0, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 30000);
                if (m_bIsRegularMode && pLifeSpark)
                {
                    pLifeSpark->SetMaxHealth(50400);
                    pLifeSpark->SetHealth(pLifeSpark->GetMaxHealth());
                }
        ...
        if (!m_bPhase2 && m_creature->GetHealthPercent() < m_uiHealthPercent && !m_bIsHardMode)
        {
            ...
            m_uiPummellerTimer      = 5000;
            m_uiMaxScrapbot         = float(m_bIsRegularMode ? 1 : 2.5f)*(urand(7, 10) * 5);
            m_uiMaxBoombot          = float(m_bIsRegularMode ? 1 : 2.5f)*(urand(3, 7) * 5);
            m_uiMaxPumeller         = m_bIsRegularMode ? 2 : 5;
            m_uiScrapbotCount       = 0;
            m_uiBoombotCount        = 0;   
            m_uiPummellerCount      = 0;
