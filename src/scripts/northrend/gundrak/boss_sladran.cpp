...
struct MANGOS_DLL_DECL mob_sladran_summon_targetAI : public ScriptedAI
{
    ...
    void JustSummoned(Creature* pSummoned)
    {
        ...
        if (Creature* pSladran = GetClosestCreatureWithEntry(m_creature, CREATURE_SLAD_RAN, 150.0f))
        {
...
struct MANGOS_DLL_DECL boss_sladranAI : public ScriptedAI
{
    ...
    void Aggro(Unit* pWho)
    {
        ...
        if (m_pInstance)
            m_pInstance->SetData(DATA_SLAD_RAN_EVENT, IN_PROGRESS);
    }
    ...
    void JustDied(Unit* pKiller)
    {
        ...
        if (m_pInstance)
            m_pInstance->SetData(DATA_SLAD_RAN_EVENT, DONE);
    }
