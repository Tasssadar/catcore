...
void MotionMaster::MoveWaypoint()
{
    if (i_owner->GetTypeId() == TYPEID_UNIT)
    {
        ...
        {
            sLog.outError("Creature %s (Entry %u) attempt to MoveWaypoint() but creature is already using waypoint", i_owner->GetObjectGuid().GetString().c_str(), i_owner->GetEntry());
            return;
        }

        Creature* creature = (Creature*)i_owner;

        DEBUG_FILTER_LOG(LOG_FILTER_AI_AND_MOVEGENSS, "Creature %s (Entry %u) start MoveWaypoint()", i_owner->GetObjectGuid().GetString().c_str(), i_owner->GetEntry());
        Mutate(new WaypointMovementGenerator<Creature>(*creature));
    }
    else
    {
        sLog.outError("Non-creature %s attempt to MoveWaypoint()", i_owner->GetObjectGuid().GetString().c_str());
    }
}
