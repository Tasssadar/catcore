...
void BattleGroundQueue::Update(BattleGroundTypeId bgTypeId, BattleGroundBracketId bracket_id, uint8 arenaType, bool isRated, uint32 arenaRating)
{
    ...
    if (bg_template->isBattleGround())
    {
        ...
        if (CheckPremadeMatch(bracket_id, MinPlayersPerTeam, MaxPlayersPerTeam))
        {
            ...
            BattleGround * bg2 = NULL;//sBattleGroundMgr.CreateNewBattleGround(bgTypeId, bracketEntry, 0, false);
            if (!bg2)
            {
    ...
    else if (bg_template->isArena())
    {
        ...
        {
            BattleGround* arena = NULL;//sBattleGroundMgr.CreateNewBattleGround(bgTypeId, bracketEntry, arenaType, true);
            if (!arena)
            {
...
{
    // Create the BG
    BattleGround *bg = CreateNewBattleGround();
    switch(bgTypeId)
    {
