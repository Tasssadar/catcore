...
pAuraHandler AuraHandler[TOTAL_AURAS]=
{
    ...
    &Aura::HandleNoImmediateEffect,                         //178 SPELL_AURA_MOD_DEBUFF_RESISTANCE          implemented in Spell::EffectApplyAura and Unit::MagicSpellHitResult
...
void Aura::HandleAuraDummy(bool apply, bool Real)
{
    ...
    if (!m_permanent)
    {
        if (Unit* caster = GetCaster())
        {
            SpellEntry const* m_spellInfo = GetSpellProto();

            for(uint8 i = 0; i < MAX_EFFECT_INDEX; ++i)
            {
                if (m_spellInfo->EffectApplyAuraName[i] == SPELL_AURA_PERIODIC_TRIGGER_SPELL ||
                    m_spellInfo->EffectApplyAuraName[i] == SPELL_AURA_PERIODIC_TRIGGER_SPELL_WITH_VALUE)
                {
                    if (apply)
                    {
                ...
                else if (m_spellInfo->EffectApplyAuraName[i] == SPELL_AURA_DUMMY && GetEffIndex() != i)
        ...
        case SPELLFAMILY_DRUID:
        {
            ...
            if (GetSpellProto()->SpellFamilyFlags & UI64LIT(0x1000000000))
            {
                ...
                else
                {
                    ...
                    if (!(GetAuraDuration() <= 0/* || m_removeMode == AURA_REMOVE_BY_DISPEL*//*lifebloom dispel has special handling in RemoveSingleAuraDueToSpellByDispel*/))
                        return;
...
void Aura::HandleShapeshiftBoosts(bool apply)
{
    ...
    if (apply)
    {
        ...
        if (plr)
        {
            ...
            // Savage Roar
            if (form == FORM_CAT && plr->HasAura(52610))
                target->CastSpell(target, 62071, true);
...
void Aura::PeriodicTick()
{
    ...
    switch(m_modifier.m_auraname)
    {
        ...
        case SPELL_AURA_PERIODIC_DAMAGE_PERCENT:
        {
            ...
            target->CalculateDamageAbsorbAndResist(pCaster, GetSpellSchoolMask(spellProto), DOT, pdamage, &absorb, &resist, true);
...
void Aura::HandleAuraOpenStable(bool apply, bool Real)
{
    ...
    if (!caster || caster->GetTypeId() != TYPEID_PLAYER || !caster->IsInWorld() || !caster->GetMap() || !((Player*)caster)->GetSession())
        return;
