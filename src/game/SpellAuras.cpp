    ...
    switch(GetSpellProto()->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_DRUID:
        {
            ...
            if (GetSpellProto()->SpellFamilyFlags & UI64LIT(0x1000000000))
            {
                ...
                {
                    // Final heal only on dispelled or duration end, using AURA_REMOVE_BY_DEFAULT because dummy aura can not get dispeled and it is probably removed somehow else
                    if (!(GetAuraDuration() <= 0 || m_removeMode == AURA_REMOVE_BY_DISPEL || m_removeMode == AURA_REMOVE_BY_DEFAULT))
                        return;
