...
void Player::Update( uint32 p_time )
{
    ...
    if (hasUnitState(UNIT_STAT_MELEE_ATTACKING))
    {
        ...
        if (pVictim && pVictim->IsInWorld() && !IsNonMeleeSpellCasted(false))
        {
            ...
            if (resultMail)
            {
                do
                {
                    ...
                    if (has_items)
                    {
                        ...
                        //                                                          0         1
                        QueryResult *resultItems = CharacterDatabase.PQuery("SELECT item_guid,item_template FROM mail_items JOIN item_instance ON item_guid = guid WHERE mail_id='%u'", mail_id);
                        if (resultItems)
                        {
                            do
                            {
                                ...
                                if (!pItem->LoadFromDB(item_guidlow, MAKE_NEW_GUID(guid, 0, HIGHGUID_PLAYER), item_template))
                                {
...
Item* Player::EquipItem( uint16 pos, Item *pItem, bool update )
{
    ...
    if ( !pItem2 )
    {
        ...
        if (isAlive())
        {
            ...
            if ((slot == EQUIPMENT_SLOT_MAINHAND || slot == EQUIPMENT_SLOT_OFFHAND || slot == EQUIPMENT_SLOT_RANGED) && pProto && isInCombat())
            {
        ...
        if ( bag == INVENTORY_SLOT_BAG_0 )
        {
            if ( slot < INVENTORY_SLOT_BAG_END )
            {
                ...
                if (slot < EQUIPMENT_SLOT_END)
                {
                    ...
                    // Apply CD on spells at unequiping from weapon slot
                    if (isAlive())
                    {
                        if ((slot == EQUIPMENT_SLOT_MAINHAND || slot == EQUIPMENT_SLOT_OFFHAND || slot == EQUIPMENT_SLOT_RANGED) && pProto && isInCombat())
                        {
...
bool Player::LoadFromDB( uint32 guid, SqlQueryHolder *holder )
{
    ...
    //"health, power1, power2, power3, power4, power5, power6, power7, specCount, activeSpec, exploredZones, equipmentCache, ammoId, knownTitles, actionBars, matchmaker_rating, rpState FROM characters WHERE guid = '%u'", GUID_LOPART(m_guid));
    ...
    SetRpState(RpState(fields[66].GetUInt8()));
...
{
    //QueryResult *result = CharacterDatabase.PQuery("SELECT bag,slot,item,item_template FROM character_inventory WHERE character_inventory.guid = '%u' ORDER BY bag,slot", GetGUIDLow());
    ...
    if (result)
    {
        ...
        do
        {
            ...
            if (!item->LoadFromDB(item_guid, GetGUID(), item_id))
            {
...
void Player::_LoadMailedItems(QueryResult *result)
{
    // data needs to be at first place for Item::LoadFromDB
    //         0        1          2
    // "SELECT mail_id, item_guid, item_template FROM mail_items WHERE receiver = '%u'", GUID_LOPART(m_guid)
    if (!result)
        return;
    ...
    do
    {
        ...
        if (!item->LoadFromDB(item_guid_low, 0, item_template))
        {
    ...
    for( QuestStatusMap::iterator i = mQuestStatus.begin( ); i != mQuestStatus.end( ); ++i )
    {
        switch (i->second.uState)
        {
            case QUEST_NEW :
                ...
                break;
            case QUEST_CHANGED :
                CharacterDatabase.PExecute("UPDATE character_queststatus SET status = '%u',explored = '%u',timer = '" UI64FMTD "',mobcount1 = '%u',mobcount2 = '%u',mobcount3 = '%u',mobcount4 = '%u',itemcount1 = '%u',itemcount2 = '%u',itemcount3 = '%u',itemcount4 = '%u'  WHERE guid = '%u' AND quest = '%u' ",
...
void Player::ResetInstances(uint8 method, bool isRaid)
{
    ...
    for (BoundInstancesMap::iterator itr = m_boundInstances[diff].begin(); itr != m_boundInstances[diff].end();)
    {
        ...
        if (!entry || entry->IsRaid() != isRaid || p->IsPermanent())
        {
            ++itr;
            continue;
        }
