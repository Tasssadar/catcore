...
uint32 Unit::DealDamage(Unit *pVictim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellEntry const *spellProto, bool durabilityLoss, uint32 absorb)
{
    ...
    if (health <= damage)
    {
        ...
        else                                                // creature died
        {
            ...
            if (cVictim->IsLoggable())
                cVictim->LogKill(this);
...
void Unit::CalculateAbsorbResistBlock(Unit *pCaster, SpellNonMeleeDamage *damageInfo, SpellEntry const* spellProto, WeaponAttackType attType)
{
    ...
    CalculateDamageAbsorbAndResist(pCaster, GetSpellSchoolMask(spellProto), SPELL_DIRECT_DAMAGE, absorb_affected_damage, &damageInfo->absorb, &damageInfo->resist, true);
    damageInfo->damage-= damageInfo->absorb + damageInfo->resist;
}
    ...
    for(AuraList::const_iterator iter = auraList.begin(); iter!=auraList.end(); ++iter)
    {
        if ((*iter)->GetSpellProto()->SpellFamilyName == SPELLFAMILY_ROGUE && (*iter)->GetSpellProto()->SpellIconID == 1960)
        {
            ...
            for(AuraMap::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
            {
                ...
                {
                    crit += (*iter)->GetSpellProto()->CalculateSimpleValue(EFFECT_INDEX_0);
                    break;
                }
            }
            break;
...
// DEVELOPER CODE END
        ...
        bool is_spellSpecPerTargetPerCasterNotStack = IsSingleFromSpellSpecificPerCasterNoStack (spellId_spec,i_spellId_spec);
...
void Unit::SendHealSpellLog(Unit *pVictim, uint32 SpellID, uint32 Damage, uint32 OverHeal, bool critical, uint32 absorb)
{
    ...
    data << uint32(absorb);                                 // absorb, look 301 aura
...
uint32 Unit::SpellDamageBonusTaken(Unit *pCaster, SpellEntry const *spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack)
{
    ...
    if (IsAreaOfEffectSpell(spellProto))
    {
        ...
        if (GetTypeId() == TYPEID_UNIT && ((Creature*)this)->isPet() && pCaster->GetTypeId() != TYPEID_PLAYER)
    ...
    for(ProcTriggeredList::const_iterator i = procTriggered.begin(); i != procTriggered.end(); ++i)
    {
        ...
        if (triggeredByAura->GetId() == 59725)
            if (Unit* pImpSRCaster = triggeredByAura->GetCaster() )
                if (pImpSRCaster->GetTypeId()==TYPEID_PLAYER)
...
void Unit::NearTeleportTo(Coords coord, float orientation, bool casting)
{
    if (GetTypeId() == TYPEID_PLAYER)
        ((Player*)this)->TeleportTo(GetMapId(), coord.x, coord.y, coord.z, orientation, TELE_TO_NOT_LEAVE_TRANSPORT | TELE_TO_NOT_LEAVE_COMBAT | TELE_TO_NOT_UNSUMMON_PET | (casting ? TELE_TO_SPELL : 0));
    else
    {
        ExitVehicle();
...
bool Unit::CanCharge(Unit *target, float x, float y, float z, float maxElev, float maxDiff)
{
    ...
    if(!GetTerrain()->VmapLoaded(cx, cy))
    {
        float tmpZ = GetTerrain()->GetHeight(cx, cy, cz, false);
        float tmpZt = GetTerrain()->GetHeight(x, y, z, false);
