...
//              damage = m_caster->SpellDamageBonus(m_caster, m_spellInfo,uint32(damage > 0 ? damage : 0), SPELL_DIRECT_DAMAGE);
        ...
        case SPELLFAMILY_DRUID:
        {
            ...
            if (m_spellInfo->SpellFamilyFlags2 & 0x00000100)
            {
                ...
                if (m_caster->hasUnitState(UNIT_STAT_CAN_NOT_REACT) || unitTarget->HasAuraType(SPELL_AURA_MOD_STEALTH))
                    return;
...
void Spell::EffectEnergize(SpellEffectIndex eff_idx)
{
    ...
    switch (m_spellInfo->Id)
    {
        ...
        case 48542:                                         // Revitalize (mana restore case)
            damage = unitTarget->GetMaxPower(POWER_MANA) / 100;
            break;
        case 71132:
    ...
    switch(m_spellInfo->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            switch(m_spellInfo->Id)
            {
                ...
                case 42924:
                {
                    if (m_caster->HasAura(43332) || !m_caster->HasAura(43880))
                        return;
            ...
            switch(m_spellInfo->Id)
            {
                case 53209:                                 // Chimera Shot
                {
                    ...
                    for(Unit::AuraMap::iterator i = Auras.begin(); i != Auras.end(); ++i)
                    {
                        ...
                        else if (familyFlag & UI64LIT(0x0000000000008000))
